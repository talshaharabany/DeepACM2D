...
def get_args():
    ...
    parser.add_argument('-D_rate', '--D_rate', default=50, help='opt. drop parameter', required=False)
    parser.add_argument('-opt', '--opt', default='adam', help='opt. type', required=False)
    parser.add_argument('-a', '--a', default=0.5, help='mask coeff with image', required=False)
    parser.add_argument('-nW', '--nW', default=0, help='number os workers', required=False)
    parser.add_argument('-WD', '--WD', default=0.00005, help='weight decay', required=False)
    parser.add_argument('-nP', '--nP', default=32, help='number of contour points', required=False)
    parser.add_argument('-order', '--order', default=85, help='backbone dimension', required=False)
    parser.add_argument('-depth_wise', '--depth_wise', default=0, help='depth_wise hardnet', required=False)
    parser.add_argument('-outlayer', '--outlayer', default=3, help='number of hardnet blocks', required=False)
    parser.add_argument('-im_size', '--im_size', default=256, help='image size', required=False)
